...
function getConfigDir() {
    ...
    if (platform === 'win32') {
        // Windows: %APPDATA%\.audioprocess-config
        configDir = path.join(os.homedir(), 'AppData', 'Roaming', '.audioprocess-config');
    } else if (platform === 'darwin') {
        // macOS: ~/Library/Application Support/.audioprocess-config
        configDir = path.join(os.homedir(), 'Library', 'Application Support', '.audioprocess-config');
    } else {
        // Linux and others: ~/.config/.audioprocess-config
        configDir = path.join(os.homedir(), '.config', '.audioprocess-config');
    }
